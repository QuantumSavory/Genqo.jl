...
- `src/projectors.jl` — `HybridProjector` + `project(::GaussianState, ...)` implement photon-number-resolved detection on a pure Gaussian state. `ClickStateKet`/`ClickStateBra` are superpositions of click patterns with a bra-ket algebra (`dot`, `adjoint`, `+`, scalar `*`). `tr` gives success probability, `dot(bra, projected_state, ket)` gives density-matrix elements, `to_fock` builds a QuantumOpticsBase `Operator`. `A_matrix` constructs the Gaussian contraction kernel from the covariance matrix, folding detector loss (η) and traced-out modes into the `G` block. Compiled moment polynomials are memoized in `C_poly_cache` behind a `ReentrantLock`.
