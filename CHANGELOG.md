...
- Reformulated A matrix and associated Wick contractions to operate directly in the [α β* α* β] basis instead of [qα pα qβ pβ]. This results in an exponential speedup because it reduces the number of terms in the expanded moment polynomial from 2^N to 1.
- Modified the A matrix function to directly compute only the upper left 2Nx2N block of A⁻¹, rather than computing the entire 4Nx4N A matrix and inverting, in cases where C polynomials only contain monomials in α and β*. This results in a speedup of 2.9-3.4x on the A matrix function for N=8,12,16.
...
- Various CI tweaks.
- The legacy source modules (`tmsv`, `spdc`, `zalm`, `sigsag`) are now thin wrappers over the v2 generalized framework: each builds its source as a Gabs `GaussianState` and computes its metrics through `project`/`tr`/`dot`/`fidelity`/`duankimble` instead of hand-rolled A-matrix construction and Wick bookkeeping. The public entry points (`covariance_matrix`, `probability_success`, `fidelity`, `spin_density_matrix`) keep their signatures and return the same values to within floating-point round-off, verified against both `test/data/ground_truth.jld2` and the reference Python implementation.
- `just ground-truth` now regenerates `test/data/ground_truth.jld2` from `test/ground_truth/GenqoV1.jl`, the original v1 implementation frozen at commit 8d75cae.

### Removed

- The v1 A-matrix machinery that the legacy modules no longer use: `tools.k_function_matrix`, `tmsv.loss_matrix_pgen`, `{spdc,zalm,sigsag}.loss_bsm_matrix_fid`, `{zalm,sigsag}.loss_bsm_matrix_pgen`, `{spdc,zalm}.dmijZ`, and the per-module `moment_vector`/`moment_terms` caches. The corresponding Python wrapper methods and `genqo.k_function_matrix` are gone as well.
