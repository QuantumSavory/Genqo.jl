...
function probability_success(μ::Real, ηᵗ::Real, ηᵈ::Real, ηᵇ::Real, dark_counts::Real)::Real
    ...
    return real(Coef * (
        ηᵇ^2 * (1-dark_counts)^4 * W(moment_terms.trc, Ainv,  true, true) +
        ηᵇ * dark_counts * (1-dark_counts)^3 * W(moment_terms.click_a, Ainv,  true, true) +
        ηᵇ * dark_counts * (1-dark_counts)^3 * W(moment_terms.click_b, Ainv,  true, true) +
        dark_counts^2 * (1-dark_counts)^2 * W(moment_terms.vac, Ainv,  true, true)
    ))
end
...
function fidelity(μ::Real, ηᵗ::Real, ηᵈ::Real, ηᵇ::Real)::Real
 # Calculate the fidelity with respect to the Bell state for the photon-photon single-mode ZALM source
    ...
    # Wick terms (cached)
    Fsum =
        W(moment_terms.bell_aa, Ainv1, true, true) +
        W(moment_terms.bell_ab, Ainv1, true, true) +
        W(moment_terms.bell_ba, Ainv1, true, true) +
        W(moment_terms.bell_bb, Ainv1, true, true)
    ...
    Trc = W(moment_terms.trc, Ainv2, false, false)
