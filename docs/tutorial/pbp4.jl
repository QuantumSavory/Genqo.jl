...
function plot_pbp4_probability()
    ...
    for (i,ηi) in enumerate(η)
        if i == 1
            p = plot(μ, Pgen[:,i], label="\\eta = $ηi", xlabel="Mean Photon Number Per Mode", ylabel="Probability of Generation", title="PBP4: Probability of generation", legend=:bottomright, dpi=300, color=i)
        else
            plot!(p, μ, Pgen[:,i], label="\\eta = $ηi", color=i)
        end
    end
    p
end
...
function plot_pbp4_fidelity()
    ...
    for (i,ηi) in enumerate(η)
        if i == 1
            p = plot(μ, F[:,i], label="\\eta = $ηi", ylim=[0,1], xlabel="Mean Photon Number Per Mode", ylabel="Fidelity", title="PBP4: Fidelity", legend=:bottomright, dpi=300, color=i)
        else
...
function plot_pbp4_distillable_entanglement_rate()
    ...
    for (i,ηTi) in enumerate(ηT)
        if i == 1
            p = plot(μ, R[:,i], label="\\eta_T = $ηTi", xlabel="Mean Photon Number Per Mode", ylabel="Distillable Entanglement Rate", title="PBP4: Distillable entanglement rate", xscale=:log10, yscale=:log10, xticks=10. .^ (-4:1), yticks=10. .^ (-14:2:-4), xlim=[1e-4,1e1], ylim=[1e-14,1e-4], legend=:topleft, dpi=300, color=i)
        else
...
@time plot_pbp4_distillable_entanglement_rate()
